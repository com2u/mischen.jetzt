...
const mix = (a,b,f)=>Array.isArray(a)?a.map((x,i)=>mix(x,b[i],f)):a*(1-f)+b*f;
...
const randConf = _=>[...Array(8)].map((_,i)=>(Math.random()*255|0+1)>>i&1).sort(_=>.5-Math.random());
...
void main() {
  ...
  //changing these can produce nicer rainbows, but it can also freeze your browser ¯\_(ツ)_/¯
  #define IRV .02
  #define IRD .125
...
function draw() {
  ...
  if(((t/cycle)|0) != (((t+=deltaTime/1000)/cycle)|0)) {
    confQueue.shift();
  ...
  sh.setUniform("pd", [w,h].map(x=>1/x));
  sh.setUniform("t", t);
